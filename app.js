...
function saveSnapshot() {
  ...
  state.snapshots.unshift(snapshot); state.snapshots = state.snapshots.slice(0, 12); persist(); renderSnapshots(); showToast("기록을 저장했어요.");
}
...
function addCheckin() {
  ...
  persist(); $("checkinMessage").textContent = "오늘의 기록을 남겼어요. 꾸준함이 가장 멋진 변화예요."; $("checkinWeight").value = ""; renderPreview(); updateGiftProgress(); showToast("체크인을 저장했어요.");
}
...
function init() {
  ...
  $("saveSnapshot").addEventListener("click", saveSnapshot); $("addCheckin").addEventListener("click", addCheckin); $("cartButton").addEventListener("click", openCart); $("closeCart").addEventListener("click", closeCart); $("drawerBackdrop").addEventListener("click", closeCart); $("checkoutButton").addEventListener("click", checkout); $("giftButton").addEventListener("click", openGift); $("closeGift").addEventListener("click", closeGift); $("closeGiftCta").addEventListener("click", closeGift); $("modalBackdrop").addEventListener("click", closeGift);
  $("clearSnapshots").addEventListener("click", () => document.querySelector("#records")?.scrollIntoView({ behavior: "smooth" }));
  document.addEventListener("keydown", (event) => { if (event.key === "Escape") { closeCart(); closeGift(); } });
}
